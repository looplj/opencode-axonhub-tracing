...
├── dist/                 # Build output (gitignored, force-added for publish)
│   ├── index.js
│   └── index.d.ts
...
| Build output | `dist/` | gitignored but force-added for npm publish |
...
- `dist/` is in `.gitignore` but must be force-added (`git add -f dist/`) for npm publish since `"files": ["dist"]`
